...
UI_Layout widget_scrollable_panel_start(UI_Rect rect, int elem_height, int elem_count, Scroll_Panel_State *state) {
    ...
    UI_Rect scrollbar_rect = {
        rect.x + rect.w - PANEL_SCROLLBAR_SIZE + m,
        rect.y,
        PANEL_SCROLLBAR_SIZE - m,
        rect.h,
    };
    ...
    if (height > rect.h) {
        ...
        knob_rect = (UI_Rect) {
            .x = scrollbar_rect.x,
            .y = scrollbar_rect.y + ((scrollbar_rect.h - knob_height) * knob_scroll),
            .w = scrollbar_rect.w,
            .h = knob_height,
        };
    }
            ...
            for (size_t y = 0; y < pattern.height; ++y) {
                for (size_t x = 0; x < pattern.width; ++x) {
                    ...
                    if (pattern.cells[y * pattern.width + x] == STATE_ALIVE) {
                        Rectangle rect = {
                            .x = board_x + bx * cell_size,
                            .y = board_y + by * cell_size,
                            .width = cell_size,
                            .height = cell_size,
                        };
